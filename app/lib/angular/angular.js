...
function angularInit(element, bootstrap) {
  window.console.time('ng.angularInit');
  ...
  window.console.timeEnd('ng.angularInit');
...
function bootstrap(element, modules) {
  window.console.time('ng.bootstrap');
  var doBootstrap = function() {
    ...
    window.console.timeEnd('ng.bootstrap');
 ...
 */
var version = {
  full: '1.2.0-280e33d',    // all of these placeholder strings will be replaced by grunt's
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    Scope.prototype = {
      ...
      $digest: function() {
        ...
        $rootScope.$digestLoops = 0;
        var loopUpdating = true;

        do { // "while dirty" loop
          if (loopUpdating)
            $rootScope.$digestLoops++;
          if ($rootScope.$digestLoops > 4) {
            loopUpdating = false;
            $rootScope.$digestLoops = "5+";
          }
          // Count the number of watchers before running them.
          var numWatchers = 0;
          ...
          $rootScope.$numDigestWatchers = numWatchers;
              ...
              while (length--) {
                try {
                  ...
                  if (watch && (value = watch.get(current)) !== (last = watch.last) &&
                    ...
                    if (watch.exp != '$digestLoops') {
                      dirty = true;
                    }
                    console.log('was dirty: ' + watch.exp + ' ' + value);
    ...
    function beginPhase(phase) {
      ...
      window.console.time('ng.scope.' + phase);

      var callsKey = "$calls_" + $rootScope.$$phase;
      window.console.log('log: ' + callsKey + ' -> ' + $rootScope[callsKey]);
      if ($rootScope[callsKey] !== undefined) {
        $rootScope[callsKey]++;
      } else {
        $rootScope[callsKey] = 0;
      }
    }
